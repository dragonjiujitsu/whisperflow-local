...
(`lightning-whisper-mlx` using the active performance profile on Apple
Silicon/Metal) -> cleanup (an authenticated local Unsloth service started and
owned by the app) -> focus-checked clipboard paste with `Cmd+V`. A
...
The active performance profile selects the STT model and batch size used by both
live dictation and self-test. STT runs through `lightning-whisper-mlx` on
MLX/Metal.

Default cleanup: Unsloth Studio serving the cached
`unsloth/Qwen3.5-4B-MTP-GGUF` model as `default` on `http://localhost:8888/v1`.
`run` starts and authenticates this service, rejects pre-existing listeners on
the configured endpoint, passes the generated key directly to the cleanup
client, and stops only its own process. If cleanup is unavailable, dictated text
is preserved for recovery instead of being sent to another backend.
...
`run` owns the warm local cleanup-service lifecycle and its runtime credential.

`doctor` checks microphone input, macOS permissions, the configured STT backend,
local cleanup configuration and model presence, clipboard access, and
hotkey/overlay imports. It never authenticates to or probes an existing cleanup
listener. On macOS, Accessibility and Microphone prompts may appear the first
